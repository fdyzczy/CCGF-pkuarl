...
for tt = 1:length(traj_file_list)
    ...
    for ff = 1:length(filter_list)
        for mm = 1:length(method_list)
            ...
            rmse_list{tt, ff, mm} = rmse_single;
            variance_list{tt, ff, mm} = variance_single;
            comptime_list{tt, ff, mm} = totalTime / (K + 1);
            violProb_list{tt, ff, mm} = violProb_single;

            rmse_x_all{tt, ff, mm} = rmse_x_total;
            rmse_theta_all{tt, ff, mm} = rmse_theta_total;
            rmse_v_all{tt, ff, mm} = rmse_v_total;
            variance_all{tt, ff, mm} = variance_total;
            compTime_all{tt, ff, mm} = calTime * 1000;
            violProb_all{tt, ff, mm} = vioProb_total;
            violProb_all_max{tt, ff, mm} = vioProb_max;
        end
    end
end

table_col = cellstr(method_list);
show_results(compTime_all, violProb_all, violProb_all_max, ...
    TT, FF, MM, table_col, filter_list, ...
    rmse_x_all, rmse_theta_all, rmse_v_all);

function show_results(compTime_all, violProb_all, violProb_all_max, ...
    TT, FF, MM, table_col, filter_list, ...
    ...
    compTime_mat = cell2mat(reshape(compTime_all, TT, FF, MM));
    violProb_mat = cell2mat(reshape(violProb_all, TT, FF, MM));
    violProbMax_mat = cell2mat(reshape(violProb_all_max, TT, FF, MM));
    rmse_x_mat = cell2mat(reshape(rmse_x_all, TT, FF, MM));
    rmse_theta_mat = cell2mat(reshape(rmse_theta_all, TT, FF, MM));
    rmse_v_mat = cell2mat(reshape(rmse_v_all, TT, FF, MM));
    ...
    if FF == 1
        avgCompTime = reshape(avgCompTime, 1, MM);
        avgViolProb = reshape(avgViolProb, 1, MM);
        avgViolProbMax = reshape(avgViolProbMax, 1, MM);
        avgRMSE_x = reshape(avgRMSE_x, 1, MM);
        avgRMSE_theta = reshape(avgRMSE_theta, 1, MM);
        avgRMSE_v = reshape(avgRMSE_v, 1, MM);
    end
    ...
    T_rmse_x = array2table(avgRMSE_x, 'VariableNames', table_col, 'RowNames', cellstr(filter_list));
    T_rmse_theta = array2table(avgRMSE_theta, 'VariableNames', table_col, 'RowNames', cellstr(filter_list));
    T_rmse_v = array2table(avgRMSE_v, 'VariableNames', table_col, 'RowNames', cellstr(filter_list));
